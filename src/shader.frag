// Copied and modified: https://github.com/niavok/glsl-factory/blob/master/shaders/mandelbulb/f.glsl
...
void main(void)
{
    ...
    // p = abs(rot(p, time * .02)) * .7;
    // p *= rot(p, sin(time *.00002));// * rot(p, length(p));
    p -= rot(p, length(p) * 1.8 - time * .5);
    p *= 2.;
    vec2 s = p;
    ...
    float ct = cos(time / 20.);
    float st = sin(time / 20.);
    float r = 1.5 + ct * .2;
    vec3 campos = vec3( r*st, 0.3-0.4*st, r*ct );
    ...
    // ray dir
    vec3 rd = normalize( s.x*cu + s.y*cv + 2.*cw );
    ...
    {
     	// rgb = 1.3*vec3(.0,.98,0.9)*(0.7+0.3*rd.y);
        rgb = vec3(0, 1, 1);
    }
    else
    {
        ...
        // ambient occlusion
        float ao = clamp(1.25*col.w-.4,0.0,1.0);
        // ao=ao*ao*0.5+0.5*ao;
        ...
        // material color
        rgb = vec3(.0,1.0,1.0);
        rgb = mix( rgb, vec3(-0.8,-10.2,0.8), sqrt(col.x)*.25 );
        rgb = mix( rgb, vec3(100.8,10.3,10.3), sqrt(col.y)*.25 );
        rgb = mix( rgb, vec3(0.7,-10.4,100.3), sqrt(col.z)*.25 );
